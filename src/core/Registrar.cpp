...
bool Registrar::IsRegisteredEdge(const std::string& lhsID, const std::string& rhsID) const {
...
size_t Registrar::GetEdgeIdx(const std::string& lhsID, const std::string& rhsID) const {
	size_t lhs = GetNodeIdx(lhsID);
	size_t rhs = GetNodeIdx(rhsID);
...
Registrar& Registrar::RegisterNodeAttr(size_t nodeIdx, std::string key, std::string value) {
	nodeAttrs[nodeIdx].emplace(std::move(key), std::move(value));
	return *this;
}

Registrar& Registrar::RegisterEdgeAttr(size_t edgeIdx, std::string key, std::string value) {
	edgeAttrs[edgeIdx].emplace(std::move(key), std::move(value));
	return *this;
}

Registrar& Registrar::DeregisterNodeAttr(size_t nodeIdx, const std::string& key) {
	nodeAttrs[nodeIdx].erase(key);
	return *this;
}

Registrar& Registrar::DeregisterEdgeAttr(size_t edgeIdx, const std::string& key) {
